...
class FW_Ext_Mailer_Sender
{
        ...
        public function get_prepared_config()
        {
                ...
                $conf   = false;
                $method = trim((string) ($settings['method'] ?? ''));
                switch ($method) {
                        case 'wpmail':
                                $conf = array();
                                break;
                        case 'smtp':
                                ...
                                $host           = trim((string) ($smtp_settings['host'] ?? ''));
                                $username       = trim((string) ($smtp_settings['username'] ?? ''));
                                $password       = trim((string) ($smtp_settings['password'] ?? ''));
                                ...
                                if (
                                        ...
                                        $conf = array(
                                                ...
                                                'port'      => trim((string) ($smtp_settings['port'] ?? ''))
                                        );
                ...
                if (false !== $conf) {
                        $conf = array_merge($conf, array(
                                'from_address'  => trim((string) ($settings['general']['from_address'] ?? '')),
                                'from_name'     => trim((string) ($settings['general']['from_name'] ?? '')),
                                '_method'       => $method
                        ));
                }
        ...
        private function send_through_wpmail($to, $subject, $message, $config)
        {
                ...
                $headers[] = "From:". htmlspecialchars($config['from_name'], ENT_QUOTES, 'UTF-8')
                        ." <". htmlspecialchars($config['from_address'], ENT_QUOTES, 'UTF-8') .">";
